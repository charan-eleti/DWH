...
import com.yeti.dwh.edifice.HDFSUtil
...
object satisfaction {
  ...
  //Usage:
    ...
    */
    ...
    val year_day = fileName(0).splitAt(9)._2.splitAt(8)._1
    //val loadDateTime = new SimpleDateFormat("yyyy-MM-dd HH:MM:SS").format(Calendar.getInstance().getTime)
    val spark = SparkSession.builder.master("local").appName("satisfaction").getOrCreate()
    ...
    val outputDF = filtered.withColumn("year_day", lit(year_day))
